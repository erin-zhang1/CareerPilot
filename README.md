...
### `searches.yaml`
Job search queries, target titles, locations, boards, and deterministic filters. Supported hard-filter keys are:

- `exclude_titles`: reject matching title substrings before any allow-list.
- `title_require_any`: optional title allow-list; blank titles pass for later enrichment.
- `location_accept` / `location_reject_non_remote`: location substring rules; remote jobs bypass these ordinary geo rules.
